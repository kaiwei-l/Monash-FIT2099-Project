...
public class SWGridTextInterface implements GridRenderer {
	...
	private static SWUniverse universe;
	 ...
	 * 
	 * @param 	newUniverse: Contains multiple grids of the SWWorld can will return the latest one
	 * @pre 	grid should not be null 
	 */
	public SWGridTextInterface(SWUniverse newUniverse) {
		SWGridTextInterface.universe = newUniverse;
	...
	public void displayMap() {
		...
		String buffer = "\n";
		final int gridHeight = universe.getCurrentGrid().getHeight();
		final int gridWidth  = universe.getCurrentGrid().getWidth();
		
	
		for (int row = 0; row< gridHeight; row++){ //for each row
			for (int col = 0; col< gridWidth; col++){ //each column of a row
				
				SWLocation loc = (SWLocation) universe.getCurrentGrid().getLocationByCoordinates(col, row);
