...
public class Take extends SWAffordance {
	...
	public void act(SWActor a) {
		if(shouldDo(a) == false) {return;}

		if (target instanceof SWEntityInterface) {
			
			// if target is a lightsaber, then we should set its owner
			if(target instanceof LightSaber)
			{
				LightSaber aLightSaber = (LightSaber) target;
				aLightSaber.setOwner(a);
			}
	...
	private boolean shouldDo(SWActor a)
	{
		return a.getItemCarried()==null;
	}
